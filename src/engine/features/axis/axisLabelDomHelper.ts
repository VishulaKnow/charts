...
export class AxisLabelHelper {
	...
	public static cropLabels(
		...
		const axisTextBlocks = block
			...
			.selectAll<SVGGraphicsElement, unknown>("text");
	...
	public static alignLabelsInKeyAxis(
		axisOptions: AxisModelOptions,
		axisElement: Selection<SVGGElement, unknown, HTMLElement, any>
	): void {
	...
	public static alignHorizontalAxisLastLabel(
		block: Block,
		maxLabelSize: number,
		axisOptions: AxisModelOptions,
		blockSize: Size,
		...
		const lastTick = block
			.getSvg()
			.select(`.${axisOptions.cssClass}`)
			.select<SVGGraphicsElement>(".tick:last-of-type");
	...
	private static getTranslateNumber(
		maxLabelSize: number,
		lastLabel: Selection<SVGGraphicsElement, unknown, HTMLElement, any>,
		marginRight: number
	) {
