...
export class Area {
	...
	private updateGroupedPath(
		block: Block,
		areaObject: Selection<BaseType, any, BaseType, any>,
		areaGenerator: IArea<MdtChartsDataRow>,
		newData: MdtChartsDataRow[]
	): Promise<any> {
		return new Promise((resolve) => {
			...
			let areaHandler: Selection<BaseType, any, BaseType, any> | Transition<BaseType, any, BaseType, any> =
				areaObject;
	...
	private updateSegmentedPath(
		block: Block,
		areasObjects: Selection<BaseType, any, BaseType, any>,
		areaGenerator: IArea<MdtChartsDataRow>
	): Promise<any> {
		return new Promise((resolve) => {
	...
	private setChartFillStyle(
		chart: TwoDimensionalChartModel,
		path: Selection<BaseType, unknown, BaseType, unknown>,
		valueIndex: number
	): void {
	...
	private renderBorderLine(
		lineGeneratorFactory: LineGeneratorFactory,
		block: Block,
		chart: TwoDimensionalChartModel,
		data: MdtChartsDataRow[],
		field: ValueField,
	...
	private renderBorderLineSegmented(
		generatorFactory: LineGeneratorFactory,
		block: Block,
		data: MdtChartsDataRow[],
		chart: TwoDimensionalChartModel
	): void {
	...
	private updateArea(
		areaGeneratorFactory: AreaGeneratorFactory,
		chartGroup: Selection<SVGGElement, any, BaseType, any>,
		block: Block,
		field: Field,
		chart: TwoDimensionalChartModel,
		newData: MdtChartsDataRow[],
	...
	private getAllAreasWithNewData(
		stackedData: StackedDataFull,
		block: Block,
		chart: TwoDimensionalChartModel,
		lineClass: string
	): Selection<SVGRectElement, StackedDataRow[], SVGGElement, any> {
		return block.svg
			.getChartGroup(chart.index)
			.selectAll<SVGRectElement, MdtChartsDataRow[]>(
				`path.${lineClass}${Helper.getCssClassesLine(chart.cssClasses)}`
			)
			.data(stackedData);
	}
