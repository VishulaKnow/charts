...
import { MdtChartsDataRow, NumberDomain } from "../../config/config";
...
describe('calculateBiggestValueAndDecremented', () => {
    ...
    test('should return biggest and biggest - 1 value for the count field', () => {
        const res = reader.calculateBiggestValueAndDecremented(repository)
    ...
    test('should return biggest and biggest - 1 value from domain in config for the count field', () => {
        domain.end = 10000;
        const res = reader.calculateBiggestValueAndDecremented(repository)
        expect(res).toStrictEqual([10000, 9999])
    });
});